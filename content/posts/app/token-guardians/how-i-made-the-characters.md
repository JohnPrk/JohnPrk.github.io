...
첫째는 캐릭터끼리였다. 판다를 만들고 나서 고양이와 강아지를 추가했는데, 새로 뽑은 캐릭터가 판다랑 결이 달랐다. 같은 앱에 나란히 들어갈 친구들인데 화풍이 따로 놀았다. 하나는 매끈한 3D인데 다른 하나는 그림체가 아예 따로 노는 식이었다.

![왼쪽은 3D로 정착한 판다, 오른쪽은 같은 작업에서 결이 따로 논 먹그림체 강아지](/images/token-guardians/style-mismatch.png)
...
강아지가 그랬다. 처음엔 꽤 괜찮게 나왔다. 판다, 고양이와 같은 3D 느낌으로 갈색 강아지가 나왔다.

![판다, 고양이와 같은 3D 느낌으로 나온 갈색 강아지](/images/token-guardians/dog-3d.png)

그런데 뭔가 괜찮으면서도 아쉬워서 "다른 느낌으로 바꿔줄래?"라고 했더니, 화풍이 통째로 수채화로 바뀌어서 돌아왔다. 톤만 살짝 손보고 싶었는데 아예 다른 그림이 된 거다.

!["다른 느낌으로" 한마디에 화풍이 통째로 수채화로 바뀐 강아지](/images/token-guardians/dog-watercolor.png)
...
한계도 있었다. 강아지처럼 주둥이가 튀어나온 캐릭터는 끝까지 잘 안 나왔다. 처음엔 벡터 SVG로 만들려고 했는데, 제미나이가 그건 제대로 못 뱉어서 래스터 이미지로 방향을 틀었다.
